...
def buscar_propiedades(propiedades, precio):
    resultado = []
    for propiedad in propiedades:
        if propiedad['zona'] not in ['A', 'B', 'C']:
            continue
        if propiedad['estado'] not in ['Disponible', 'Reservado', 'Vendido']:
            continue
        if propiedad['año'] < 2000:
            continue
        if propiedad['metros'] < 60:
            continue
        if propiedad['habitaciones'] < 2:
            continue
        precio_calculado = calcular_precio(propiedad)
        if precio_calculado <= precio:
            propiedad['precio_calculado'] = precio_calculado
            resultado.append(propiedad)
    return resultado

def calcular_precio(propiedad):
    zona = propiedad['zona']
    tamaño = propiedad['metros']
    habitaciones = propiedad['habitaciones']
    garaje = 1 if propiedad['garaje'] else 0
    año = propiedad['año']
    antigüedad = 2023 - año
    precio_base = tamaño * 100 + habitaciones * 500 + garaje * 1500
    factor_precio = 1 - antigüedad / 100
    if zona == 'A':
        precio = precio_base * factor_precio
    elif zona == 'B':
        precio = precio_base * factor_precio * 1.5
    elif zona == 'C':
        precio = precio_base * factor_precio * 2
    return precio

propiedades = [{'año': 2010, 'metros': 150, 'habitaciones': 4, 'garaje': True, 'zona': 'C', 'estado': 'Disponible'},
{'año': 2016, 'metros': 80, 'habitaciones': 2, 'garaje': False, 'zona': 'B', 'estado': 'Reservado'},
{'año': 2000, 'metros': 180, 'habitaciones': 4, 'garaje': True, 'zona': 'A', 'estado': 'Disponible'},
{'año': 2015, 'metros': 95, 'habitaciones': 3, 'garaje': True, 'zona': 'B', 'estado': 'Vendido'},
{'año': 2008, 'metros': 60, 'habitaciones': 2, 'garaje': False, 'zona': 'C', 'estado': 'Disponible'}]
...
while True:
    ...
    print('5. Cambiar estado de inmueble') #Seleccione indice 0, 1, 2, ...
    ...
    if opcion == 1:
        ...
        print('Inmuebles seleccionados:', seleccion)
    elif opcion == 2:
